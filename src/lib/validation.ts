...
export const computerSchema = z.object({
  ...
  labId: z.string().min(1, "Invalid Lab selection"),
  ...
  motherboard: z.string().min(1).default(""),
  ramGb: z.number().int().positive("RAM size must be positive"),
  ssdGb: z.number().int().nonnegative(),
  hddGb: z.number().int().nonnegative(),
  gpu: z.string().optional(),
  monitorDetails: z.string().min(1).default(""),
  keyboardBrand: z.string().min(1).default(""),
  mouseBrand: z.string().min(1).default(""),
  upsDetails: z.string().optional(),
  purchaseDate: z.coerce.date(),
  warrantyExpiry: z.coerce.date(),
  invoiceUrl: z.string().optional().nullable(),
  vendorDetails: z.string().min(1).default(""),
  operatingSystem: z.string().min(2, "Operating system is required"),
  windowsKey: z.string().optional().nullable(),
  biosVersion: z.string().optional().nullable(),
  condition: z.enum(["EXCELLENT", "WORKING", "FAULTY", "SCRAP"]).default("EXCELLENT"),
  status: z.enum(["ONLINE", "OFFLINE", "UNDER_REPAIR", "DECOMMISSIONED"]).default("ONLINE"),
